"""Tests for :func:`lidar_scan.apply_tile_pyramid`."""
...
from lidar_scan import apply_tile_pyramid
...
def test_function_exported_from_module_and_package():
    assert tiles_module.apply_tile_pyramid is apply_tile_pyramid
    import lidar_scan
    assert "apply_tile_pyramid" in lidar_scan.__all__
...
def test_double_empty_returns_empty_tuple():
    assert apply_tile_pyramid((), ()) == ()


def test_empty_deltas_returns_base_identical_object():
    base = ((_tile(),),)
    assert apply_tile_pyramid(base, ()) is base
...
def test_geometry_is_copied_unchanged():
    ...
    tile = apply_tile_pyramid(base, deltas)[0][0]
...
def test_level_and_tile_order_follow_base():
    ...
    deltas = (
        ...
        (_delta(ix0=0, iy0=0, ix1=511, iy1=511, dzmin=0.0, dzmax=4.0,
                dcount=2),),
    )
    result = apply_tile_pyramid(base, deltas)
    assert [t[:2] for t in result[0]] == [(0, 0), (1, 0)]
    assert [t[6:9] for t in result[0]] == [(3, 4, 5), (3, 7, 3)]
    assert result[1][0][6:9] == (9, 13, 5)
...
def test_inputs_are_not_modified_and_calls_are_stable():
    ...
    first = apply_tile_pyramid(base, deltas)
    second = apply_tile_pyramid(base, deltas)
    assert first == second
    assert base == base_before
    assert deltas == deltas_before


def test_recovers_estimate_from_reference_and_deltas():
    from lidar_scan import assess_tile_pyramid_deltas, build_tile_pyramid

    estimate = build_tile_pyramid(
        [(0.0, 0.0, 3.0, 1, 1.0), (300.0, 0.0, 2.0, 1, 1.0),
         (-5.0, 300.0, 7.25, 1, 1.0)], levels=2)
    reference = build_tile_pyramid(
        [(0.0, 0.0, 1.0, 1, 1.0), (300.0, 0.0, 5.0, 1, 1.0),
         (-5.0, 300.0, 7.0, 1, 1.0)], levels=2)
    deltas = assess_tile_pyramid_deltas(estimate, reference)
    assert apply_tile_pyramid(reference, deltas) == estimate


def test_zero_resulting_count_is_allowed():
    result = apply_tile_pyramid(
        ((_tile(zmin=1.0, zmax=2.0, count=0),),),
...
def test_level_not_tuple_raises_value_error():
    with pytest.raises(ValueError):
        apply_tile_pyramid(([_tile()],), ((_delta(),),))
    with pytest.raises(ValueError):
        apply_tile_pyramid(((_tile(),),), ([_delta()],))
...
def test_tile_wrong_container_or_length_raises_value_error(which):
    ...
    if which == "base":
        ...
        for bad in bads:
            with pytest.raises(ValueError):
                apply_tile_pyramid(bad, good_deltas)
    else:
        bads = (
            ((list(_delta()),),),
        ...
        for bad in bads:
            with pytest.raises(ValueError):
                apply_tile_pyramid(good_base, bad)
...
def test_base_integer_fields_wrong_type_raise_value_error(pos, bad_value):
    ...
    tile[pos] = bad_value
    with pytest.raises(ValueError):
        apply_tile_pyramid(((tuple(tile),),), ((_delta(),),))
...
def test_delta_integer_fields_wrong_type_raise_value_error(pos, bad_value):
    ...
    tile[pos] = bad_value
    with pytest.raises(ValueError):
        apply_tile_pyramid(((_tile(),),), ((tuple(tile),),))
...
def test_unsorted_coordinates_raise_value_error():
    ...
    with pytest.raises(ValueError):
        apply_tile_pyramid(unsorted_base, unsorted_deltas)
...
def test_duplicate_coordinates_raise_value_error():
    ...
    with pytest.raises(ValueError):
        apply_tile_pyramid(duplicate, duplicate)
...
def test_different_level_counts_raise_value_error():
    with pytest.raises(ValueError):
        apply_tile_pyramid(((_tile(),),), ((), ()))
    with pytest.raises(ValueError):
        apply_tile_pyramid((), ((_delta(),),))
...
def test_coordinate_mismatch_at_second_level_raises_value_error():
    ...
    with pytest.raises(ValueError):
        apply_tile_pyramid(base, deltas)
