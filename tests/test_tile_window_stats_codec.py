...
def test_decode_window_round_trips_against_encoder():
    ...
    text = encode_tile_window_stats(pyramid, 2, -100, -100, 400, 400)
...
def test_decode_inclusive_window_bounds_x(ix_min, ix_max):
    text = (
        ...
        '0.500000,3]]}')
    assert len(decode_tile_window_stats(text)[5]) == 1
...
def test_decode_unsorted_tiles_raise_value_error():
    bad = (
        ...
        '"tiles":[[1,0,256,0,511,255,1.000000,2.000000,1.500000,0.500000,1],'
...
def test_decode_duplicate_tile_coordinates_raise_value_error():
    bad = (
        ...
        '"tiles":[[0,1,0,256,255,511,1.000000,2.000000,1.500000,0.500000,1],'
        '[0,1,0,256,255,511,3.000000,4.000000,3.500000,0.250000,2]]}')
    with pytest.raises(ValueError):
        decode_tile_window_stats(bad)
...
def test_decode_non_intersecting_tile_raises_value_error(bounds):
    with pytest.raises(ValueError):
        decode_tile_window_stats(_doc(bounds=bounds))
