"""Tests for :func:`lidar_scan.rollback_tile_pyramid_windows`."""
...
def test_function_exported_from_module_and_package():
    assert (tiles_module.rollback_tile_pyramid_windows
            is rollback_tile_pyramid_windows)
    import lidar_scan
    assert "rollback_tile_pyramid_windows" in lidar_scan.__all__
...
def test_empty_windows_returns_empty_tuple():
    assert rollback_tile_pyramid_windows((), (), ()) == ()
    base = ((_tile(),),)
    deltas = ((_delta(),),)
    assert rollback_tile_pyramid_windows(base, deltas, ()) == ()
...
def test_empty_deltas_queries_base_unchanged():
    ...
    result = rollback_tile_pyramid_windows(base, (), ((0, 0, 0, 600, 600),))
    assert result == (
...
def test_window_without_matches_keeps_window_and_empty_tiles():
    base = ((),)
    assert rollback_tile_pyramid_windows(base, (), ((0, 0, 0, 10, 10),)) == \
        ((0, 0, 0, 10, 10, ()),)


def test_empty_levels_are_allowed():
    base = ((), (), ())
    result = rollback_tile_pyramid_windows(
...
def test_rollback_then_window_matches_expected_selection():
    base = ((
        _tile(tx=0, ty=0, zmin=10.0, zmax=20.0, count=8),
    ...
    result = rollback_tile_pyramid_windows(base, deltas, windows)
...
def test_non_integral_values_become_quantized_floats():
    ...
    tile = rollback_tile_pyramid_windows(
...
def test_inputs_are_not_modified_and_calls_are_stable():
    ...
    first = rollback_tile_pyramid_windows(base, deltas, windows)
    second = rollback_tile_pyramid_windows(base, deltas, windows)
    assert first == second
...
def test_inverts_assess_tile_pyramid_deltas_via_windows():
    ...
    result = rollback_tile_pyramid_windows(estimate, deltas, windows)
...
def test_non_tuple_outer_inputs_raise_type_error(bad):
    with pytest.raises(TypeError):
        rollback_tile_pyramid_windows(bad, (), ())
    with pytest.raises(TypeError):
        rollback_tile_pyramid_windows((), bad, ())
    with pytest.raises(TypeError):
        rollback_tile_pyramid_windows((), (), bad)
...
def test_window_fields_must_be_non_bool_ints(pos, bad_value):
    ...
    with pytest.raises(TypeError):
        rollback_tile_pyramid_windows((), (), (tuple(window),))
...
def test_level_out_of_range_raises_value_error():
    with pytest.raises(ValueError):
        rollback_tile_pyramid_windows(((), ()), (), ((2, 0, 0, 1, 1),))
    with pytest.raises(ValueError):
        rollback_tile_pyramid_windows(((), ()), (), ((-1, 0, 0, 1, 1),))


def test_inverted_window_bounds_raise_value_error():
    with pytest.raises(ValueError):
        rollback_tile_pyramid_windows(((),), (), ((0, 2, 0, 1, 1),))
    with pytest.raises(ValueError):
        rollback_tile_pyramid_windows(((),), (), ((0, 0, 2, 1, 1),))


# ---------------------------------------------------------------------------
# ValueError: base/deltas contract inherited from rollback_tile_pyramid_deltas
# ---------------------------------------------------------------------------

def test_bad_base_structure_raises_value_error():
    with pytest.raises(ValueError):
        rollback_tile_pyramid_windows(([_tile()],), ((_delta(),),), ())
