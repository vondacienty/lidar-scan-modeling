...
def test_rollback_then_window_selection():
    base = ((
        _tile(tx=0, ty=0, zmin=10.0, zmax=20.0, count=8),
    ...
    assert result == (
        (0, 256, 0, 300, 255,
         ((1, 0, 256, 0, 511, 255, 2.0, 4.0, 0),)),
    )


def test_delta_tiles_may_be_reordered_within_a_level():
    base = ((
        _tile(tx=0, ty=0, zmin=10.0, zmax=20.0, count=8),
    ...
    assert [tile[0:2] for tile in result[0][5]] == [(0, 0), (0, 1), (1, 0)]
    assert [tile[6:9] for tile in result[0][5]] == [
        (8.0, 22.0, 5), (0.5, 1.5, 1), (2.0, 4.0, 0)]


def test_result_does_not_depend_on_delta_order():
    base = ((
        _tile(tx=0, ty=0, zmin=10.0, zmax=20.0, count=8),
        _tile(tx=1, ty=0, ix0=256, ix1=511, zmin=3.0, zmax=5.0, count=4),
    ),)
    ...
    deltas_b = ((deltas_a[0][1], deltas_a[0][0]),)
    windows = ((0, 0, 0, 600, 600),)
    assert (rollback_tile_pyramid_delta_windows(base, deltas_a, windows)
            == rollback_tile_pyramid_delta_windows(base, deltas_b, windows))
...
def test_closed_intervals_match_at_endpoints():
    ...
    assert result[0][5] == ((0, 0, 0, 0, 255, 255, 1.0, 1.0, 1),)
    result = rollback_tile_pyramid_delta_windows(
        base, deltas, ((0, 256, 0, 300, 255),))
    assert result[0][5] == ()
...
def test_multiple_levels_select_the_right_level():
    ...
    assert result[0][5] == ((0, 0, 0, 0, 255, 255, 1.0, 2.0, 0),)
    assert result[1][5] == ((0, 0, 0, 0, 511, 511, 3.0, 3.0, 0),)


def test_results_are_quantized_floats_even_when_integral():
...
def test_inverts_assess_tile_pyramid_deltas_via_windows():
    ...
    expected = tuple(
        tuple((tile[0], tile[1], tile[2], tile[3], tile[4], tile[5],
               float(tile[6]), float(tile[7]), tile[8])
              for tile in level)
        for level in reference
    )
    assert tuple(window[5] for window in result) == expected
...
def test_level_out_of_range_raises_value_error():
    with pytest.raises(ValueError):
        rollback_tile_pyramid_delta_windows(((), ()), (), ((2, 0, 0, 1, 1),))
    with pytest.raises(ValueError):
        rollback_tile_pyramid_delta_windows(((), ()), (), ((-1, 0, 0, 1, 1),))


def test_inverted_window_bounds_raise_value_error():
    with pytest.raises(ValueError):
        rollback_tile_pyramid_delta_windows(((),), (), ((0, 2, 0, 1, 1),))
    with pytest.raises(ValueError):
        rollback_tile_pyramid_delta_windows(((),), (), ((0, 0, 2, 1, 1),))
...
def test_base_level_must_be_sorted_without_duplicates():
    base = ((_tile(tx=1, ty=0, ix0=256, ix1=511), _tile(tx=0, ty=0)),)
    deltas = ((_delta(tx=0, ty=0),
               _delta(tx=1, ty=0, ix0=256, ix1=511)),)
    with pytest.raises(ValueError):
        rollback_tile_pyramid_delta_windows(base, deltas, ())
...
def test_delta_tile_must_be_nine_tuple():
    with pytest.raises(ValueError):
        rollback_tile_pyramid_delta_windows(
            ((_tile(),),), (((0, 0, 0, 0, 255, 255, 0.0, 0.0),),), ())


def test_delta_fields_must_have_proper_types():
    with pytest.raises(ValueError):
        rollback_tile_pyramid_delta_windows(
            ((_tile(),),), ((_delta(dcount=True),),), ())
    with pytest.raises(ValueError):
        rollback_tile_pyramid_delta_windows(
            ((_tile(),),), ((_delta(dzmin=1),),), ())
    with pytest.raises(ValueError):
        rollback_tile_pyramid_delta_windows(
            ((_tile(),),), ((_delta(dzmax=math.inf),),), ())


def test_delta_duplicate_coordinates_raise_value_error():
    with pytest.raises(ValueError):
        rollback_tile_pyramid_delta_windows(
            ((_tile(),),), ((_delta(), _delta()),), ())


def test_delta_bounds_must_not_be_inverted():
    with pytest.raises(ValueError):
        rollback_tile_pyramid_delta_windows(
            ((_tile(),),), ((_delta(ix0=10, ix1=5),),), ())
