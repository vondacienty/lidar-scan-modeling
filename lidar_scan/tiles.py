...
def checkout_log(log: str, records: tuple) -> str:
    ...
    record's ``before`` must byte-for-byte equal the canonical encoding
    of the previous record's ``result.snapshot``.

    Returns the canonical compact JSON document with exactly the two
    top-level keys ``batches`` and ``snapshot`` in that order.
    ``batches`` preserves the record order, each row being
    ``[id, before, [plan...], result]`` with ``before``, every ``plan``
    and ``result`` embedded as JSON objects; ``snapshot`` is ``null``
    for an empty ledger and otherwise the last record's
    ``result.snapshot``. The output uses ``ensure_ascii=False``, no
    whitespace and no trailing newline, and repeated calls return a
    byte-identical document.

    :raises TypeError: ``log`` is not a ``str``, ``records`` is not a
        ``tuple``, a record is not a three-item ``tuple``, or a field
        has the wrong type.
    :raises ValueError: the log is not its canonical encoding, an ``id``
        is invalid or duplicated, ``plans`` is empty, the snapshot
        linkage is broken, or a ``before`` or plan document is not its
        canonical encoding.
    """
    if not isinstance(log, str):
        raise TypeError("log must be a str")
...
def replay_checkout(log: str, text: str) -> str:
    ...
    the canonical output of :func:`checkout_log`. The whole ledger is
    recomputed from ``log`` and the decoded records -- identifiers,
    snapshot linkage, every stored ``result`` against
    :func:`apply_delivery_checkouts` and the final ``snapshot`` -- and
    required to match ``text`` byte-for-byte.

    Returns the canonical compact JSON document with exactly the two
    top-level keys ``audit`` and ``snapshot`` in that order. ``audit``
    preserves the batch order, each row being ``[id, status]`` with
    ``status`` the verified ``result.status`` of that batch;
    ``snapshot`` is the ledger's ``snapshot`` (``null`` for an empty
    ledger). The output uses ``ensure_ascii=False``, no whitespace and
    no trailing newline, and repeated calls return a byte-identical
    document.

    :raises TypeError: ``log`` or ``text`` is not a ``str``.
    :raises ValueError: the log is not its canonical encoding, the
        ledger document is malformed or not canonical, an identifier is
        invalid or duplicated, the snapshot linkage is broken, or a
        recomputed result does not match.
    """
    if not isinstance(log, str):
        raise TypeError("log must be a str")
    ...
    parts = ['{"audit":[']
    for index, (record, result_node) in enumerate(
            zip(records, result_nodes)):
        if index:
            parts.append(",")
