...
def _decode_delivery_plan(text: str) -> list:
    ...
    Returns the operations list in document order, each operation a
    ``[product, action, current, target, affected, failures]`` row
    (``target`` is ``None`` for ``block`` actions and otherwise a version
    string, ``affected`` a list of batch ints and ``failures`` a list of
    ``[batch, version, level, ix_min, iy_min, ix_max, iy_max, R]`` with ``R``
    ``None`` or ``[emin, emax, rmse, amean, n]``).

    Beyond JSON, shape and canonical-format checks, the plan contract is
    revalidated: product rows are sorted uniquely by product name; action is
    one of ``publish``/``rollback``/``block``; a ``publish`` row targets the
    current version with empty ``affected``/``failures`` while a
    ``rollback``/``block`` row carries a non-null/null ``target`` and lists
    at least one affected batch and one failure; ``affected`` is strictly
    ascending and matches exactly the batches of the row's failures, which
    are ordered by batch; and the top-level ``releasable`` flag is true only
    when every action is ``publish`` (an empty operation set being ``true``).
    ...
    for raw_operation in raw_operations:
        if not isinstance(raw_operation, list) or len(raw_operation) != 6:
            raise ValueError("each operation must be an array of six values")
        ...
        if action not in ("publish", "rollback", "block"):
            raise ValueError("operation action must be publish, rollback or "
                             "block")
        if not isinstance(current, str):
            raise ValueError("operation current must be a str")
        if not current or not set(current) <= _DELIVERY_IDENT_CHARS:
            raise ValueError(
                "operation current must be non-empty and contain only ASCII "
                "alphanumeric characters and ._-"
            )
        if action == "block":
            if target is not None:
                raise ValueError("a block operation must have a null target")
        else:
            if not isinstance(target, str):
                raise ValueError("operation target must be a str")
            if not target or not set(target) <= _DELIVERY_IDENT_CHARS:
                raise ValueError(
                    "operation target must be non-empty and contain only "
        ...
        if any(affected[index] >= affected[index + 1]
            ...
            raise ValueError("operation affected batches must be strictly "
                             "ascending")
        if not isinstance(raw_failures, list):
            raise ValueError("operation failures must be an array")

        failures = []
        for raw_failure in raw_failures:
            if not isinstance(raw_failure, list) or len(raw_failure) != 8:
                raise ValueError("each operation failure must be an array of "
                                 "eight values")
            batch, version = raw_failure[:2]
            if isinstance(batch, bool) or not isinstance(batch, int):
                raise ValueError("operation failure batch must be a non-bool "
                                 "integer")
            if batch < 0:
                raise ValueError("operation failure batch must be "
                                 "non-negative")
            if not isinstance(version, str):
                raise ValueError("operation failure version must be a str")
            if not version or not set(version) <= _DELIVERY_IDENT_CHARS:
                raise ValueError(
                    "operation failure version must be non-empty and contain "
                    "only ASCII alphanumeric characters and ._-"
                )
            for name, value in zip(("level", "ix_min", "iy_min", "ix_max",
                                    "iy_max"), raw_failure[2:7]):
                if isinstance(value, bool) or not isinstance(value, int):
                    raise ValueError(f"operation failure {name} must be a "
                                     "non-bool integer")
            level, ix_min, iy_min, ix_max, iy_max = raw_failure[2:7]
            if level < 0:
                raise ValueError("operation failure level must be "
                                 "non-negative")
            if ix_min > ix_max or iy_min > iy_max:
                raise ValueError("operation failure bounds must satisfy "
                                 "ix_min <= ix_max and iy_min <= iy_max")
            ...
            if raw_summary is not None:
                if not isinstance(raw_summary, list) or len(raw_summary) != 5:
                    raise ValueError("operation failure R must be null or an "
                                     "array of five values")
                metrics_raw = list(raw_summary[:4])
                match_count = raw_summary[4]
                for name, value in zip(("emin", "emax", "rmse", "amean"),
                ...
                for name, value in zip(("emin", "emax", "rmse", "amean"),
                                       metrics):
                    if not value.is_finite():
                        raise ValueError(f"operation failure {name} must be "
                                         "finite")
                if isinstance(match_count, bool) or not isinstance(
                        match_count, int):
                    raise ValueError("operation failure n must be a non-bool "
                                     "integer")
                if match_count < 1:
                    raise ValueError("operation failure n must be a positive "
                                     "integer")
                if metrics[2] < 0 or metrics[3] < 0:
                    raise ValueError("operation failure rmse and amean must "
                                     "be non-negative")
                summary = [*metrics, match_count]
        ...
        # Row field association: a publish operation retargets the current
        # version and carries no failure state; a rollback/block operation
        # carries at least one affected batch and one failure, and the
        # failures expand exactly the affected batches in batch order.
        if action == "publish":
            if target != current or affected or failures:
                raise ValueError("a publish operation must target the "
                                 "current version with no affected batches "
                                 "or failures")
        else:
            if not affected or not failures:
                raise ValueError("a rollback or block operation must list an "
            ...
            if any(failure_batches[index] > failure_batches[index + 1]
                ...
                raise ValueError("operation failures must be ordered by "
                                 "batch")
            if sorted(set(failure_batches)) != affected:
                raise ValueError("operation affected batches must match the "
                                 "batches of its failures")

        operations.append([product, action, current, target, affected,
                           failures])

    # Operations must appear strictly ascending by product name; a
    # byte-for-byte re-encode alone would tolerate reordering.
    names = [operation[0] for operation in operations]
    if any(names[index] >= names[index + 1]
           for index in range(len(names) - 1)):
        raise ValueError("operations must be sorted uniquely by product name")

    if releasable != all(operation[1] == "publish"
                         for operation in operations):
        raise ValueError("'releasable' must be true only when every action "
                         "is publish")
    ...
    if _format_delivery_plan(operations, releasable) != text:
        raise ValueError("plan is not the canonical delivery plan encoding")
    return operations


def build_delivery_receipt(plan: str, results: tuple) -> str:
    """Build a per-product delivery receipt from a plan and its results.
    ...
    sorted uniquely by product name.

    ``results`` must be a ``tuple`` whose items are strictly three-value
    tuples ``(product, status, reason)`` of ``str``. The products must
    correspond one-to-one with the plan's products, without duplicates or
    unknown/missing products, but may appear in any order. A ``publish`` or
    ``rollback`` operation admits only ``succeeded`` or ``failed``; a
    ``block`` operation admits only ``blocked``. A ``succeeded`` result must
    carry an empty ``reason``; ``failed`` and ``blocked`` results must carry
    a non-empty one.

    Returns a canonical compact JSON document with exactly the two keys
    ``receipts`` and ``ready`` in that order. Receipts follow the plan order
    and each receipt is the array
    ``[product, action, current, target, affected, status, reason,
    failures]`` with the plan fields preserved verbatim. ``ready`` is true
    only when every operation is a ``publish`` with a ``succeeded`` result;
    an empty plan yields ``{"receipts":[],"ready":true}``. Strings use
    ``ensure_ascii=False``, integers are decimal, summary metrics use
    exactly six decimal places (negative zero written as ``0.000000``),
    ``NaN``/``Infinity`` never appear and the output has no whitespace or
    trailing newline. The inputs are never modified and reordering
    ``results`` yields a byte-identical document.
    ...
    :raises ValueError: the plan is not the canonical plan encoding, a
        ...
        reason violates the action-specific contract.
    """
    ...
    for item in results:
        ...
        if not all(isinstance(field, str)
                   for field in (item[0], item[1], item[2])):
            raise TypeError("result product, status and reason must be str")

    operations = _decode_delivery_plan(plan)

    outcomes = {}
    for product, status, reason in results:
        if product in outcomes:
            raise ValueError(f"result product is duplicated: {product}")
        outcomes[product] = (status, reason)

    planned_products = {operation[0] for operation in operations}
    if len(outcomes) != len(operations) or set(outcomes) != planned_products:
        raise ValueError("result products must correspond one-to-one with "
                         "the plan products")
    ...
    for index, (product, action, current, target, affected,
                failures) in enumerate(operations):
        status, reason = outcomes[product]
        if action == "block":
            allowed = ("blocked",)
        else:
        ...
        if status not in allowed:
            raise ValueError(
                f"status {status!r} is not valid for action {action!r}"
            )
        if status == "succeeded":
            if reason:
                raise ValueError("a succeeded result must have an empty "
                                 "reason")
        elif not reason:
            raise ValueError("a failed or blocked result must have a "
                             "non-empty reason")
        if action != "publish" or status != "succeeded":
            ready = False
