...
def _validate_pyramid_stats(pyramid) -> None:
    ...
    Mirrors :func:`_validate_pyramid` but expects 11-tuples
...
def query_tile_pyramid_stats(pyramid: tuple, level: int,
    ...
    """Look up the tile ``(tx, ty)`` at ``level`` of a stats tile pyramid.

    ``pyramid`` must be an outer tuple as produced by
    :func:`build_tile_pyramid_stats`: each level is a tuple of
    ``(tx, ty, ix0, iy0, ix1, iy1, zmin, zmax, zmean, zsigma, count)``
    11-tuples sorted lexicographically by ``(tx, ty)``, with the first six
    fields and ``count`` non-bool ints, the four statistics finite floats and
    ``zsigma`` positive. ``level``, ``tx`` and ``ty`` must be non-bool ints
    and ``level`` must satisfy ``0 <= level < len(pyramid)``.
