...
def update_tile_pyramid(pyramid: tuple,
    ...
    ``pyramid`` must be an outer tuple as produced by :func:`build_tile_pyramid`
    with exactly ``levels`` levels: each level is a tuple of
    ``(tx, ty, ix0, iy0, ix1, iy1, zmin, zmax, count)`` 9-tuples with strictly
    increasing ``(tx, ty)`` coordinates, where the first six fields and
    ``count`` are non-bool ints, ``zmin``/``zmax`` are finite floats and, at
    level ``l`` with ``N = tile_cells * 2 ** l``, the cell bounds are exactly
    ``(ix0, iy0, ix1, iy1) = (tx * N, ty * N, (tx + 1) * N - 1,
    (ty + 1) * N - 1)``.
