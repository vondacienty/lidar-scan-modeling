...
def _encode_region_stats_text(level: int, tx_min: int, ty_min: int,
                              tx_max: int, ty_max: int,
                              tiles: Iterable[tuple]) -> str:
    """Build the canonical compact region-stats JSON text from valid tiles."""
    parts = ["{\"level\":", str(level),
             ",\"tx_min\":", str(tx_min),
             ",\"ty_min\":", str(ty_min),
             ",\"tx_max\":", str(tx_max),
...
def encode_tile_region_stats(pyramid: tuple, level: int,
    ...
    return _encode_region_stats_text(
...
def decode_tile_region_stats(text: str) -> tuple:
    ...
    Only the encoder's canonical compact form is accepted: the top-level
    object has exactly the keys ``level``, ``tx_min``, ``ty_min``,
    ``tx_max``, ``ty_max`` and ``tiles`` in that order; the first five values
    are non-bool ints with ``level >= 0`` and non-inverted bounds; ``tiles``
    is an array of strict eleven-item arrays
    ``[tx, ty, ix0, iy0, ix1, iy1, zmin, zmax, zmean, zsigma, count]`` whose
    indices and ``count`` are non-bool ints, whose four statistics are finite
    numbers with ``zsigma > 0``, and whose tiles are strictly increasing in
    ``(tx, ty)`` (no duplicates) and lie within the echoed rectangle.

    Returns ``(level, tx_min, ty_min, tx_max, ty_max, tiles)`` where
    ``tiles`` is a tuple of eleven-tuples in the stored order. The input is
    never modified.

    :raises TypeError: ``text`` is not a ``str``.
    :raises ValueError: the JSON syntax, key order, types, bounds, ordering,
        range, non-finite values or canonical re-encoding do not match.
    """
    if not isinstance(text, str):
        raise TypeError("text must be a str")
    ...
    if not isinstance(document, dict) or tuple(document) != expected_keys:
        raise ValueError(
            "top-level value must be an object with exactly 'level', "
            "'tx_min', 'ty_min', 'tx_max', 'ty_max' and 'tiles' in that order"
        )

    level = document["level"]
    tx_min = document["tx_min"]
    ty_min = document["ty_min"]
    tx_max = document["tx_max"]
    ty_max = document["ty_max"]
    raw_tiles = document["tiles"]

    for name, value in zip(expected_keys[:5],
                           (level, tx_min, ty_min, tx_max, ty_max)):
        if isinstance(value, bool) or not isinstance(value, int):
    ...
    for raw_tile in raw_tiles:
        ...
        (tx, ty, ix0, iy0, ix1, iy1,
         zmin, zmax, zmean, zsigma, count) = tile
        for name, value in (("tx", tx), ("ty", ty), ("ix0", ix0),
                            ("iy0", iy0), ("ix1", ix1), ("iy1", iy1),
                            ("count", count)):
            if isinstance(value, bool) or not isinstance(value, int):
                raise ValueError(f"{name} must be a non-bool int")
        for name, value in (("zmin", zmin), ("zmax", zmax),
                            ("zmean", zmean), ("zsigma", zsigma)):
        ...
        if not (tx_min <= tx <= tx_max and ty_min <= ty <= ty_max):
            raise ValueError("each tile must lie within the region bounds")
        key = (tx, ty)
        if prev_key is not None and key <= prev_key:
    ...
    # duplicate keys, non-six-decimal statistic formatting, leading zeros,
    # -0, exponents and any other non-canonical spelling.
    if _encode_region_stats_text(level, tx_min, ty_min, tx_max, ty_max,
                                 tiles) != text:
        raise ValueError("JSON text is not the canonical region-stats encoding")
    return (level, tx_min, ty_min, tx_max, ty_max, tuple(tiles))
