...
def aggregate_tile_window_stats(pyramid: tuple, level: int,
    ...
    A tile matches when its closed cell-index intervals intersect the window:
    ``tile.ix1 >= ix_min and tile.ix0 <= ix_max and tile.iy1 >= iy_min and
    tile.iy0 <= iy_max``. The matched tiles (in the level's existing order)
    are aggregated: ``zmin`` is the minimum tile ``zmin``, ``zmax`` the
    maximum tile ``zmax`` and ``count`` the sum of the tile counts. The z
    statistics use weights ``w = 1 / zsigma ** 2``:
    ``zmean = sum(w * zmean) / sum(w)`` and ``zsigma = sqrt(1 / sum(w))``;
    the summations are Decimal computations (precision 50,
    ``ROUND_HALF_EVEN``, each input converted via ``Decimal(str(v))``) with
    the summands accumulated in ascending Decimal order, so the result does
    not depend on tile order. The four statistics are quantized to six
