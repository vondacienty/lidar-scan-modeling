...
def _delivery_range_result(products, product, batch_min, batch_max):
    """Range query over already-decoded snapshot product items."""
...
def plan_delivery_updates(before: str, after: str, ranges: tuple) -> str:
    """Plan the per-range update operations between two delivery snapshots.
    ...
    ``batch_min``/``batch_max`` non-bool ``int`` bounds satisfying
    ``0 <= batch_min <= batch_max``; no
    ``(product, batch_min, batch_max)`` key may repeat. Each snapshot is
    parsed exactly once.

    For each requested range, ``B`` (``base``) and ``A`` (``target``) are
    the return values of :func:`query_delivery_range` called against
    ``before`` and ``after`` respectively (``None`` when the product is
    absent).

    Returns the canonical compact JSON document
    ``{"operations":[...],"changed":...}`` with exactly these two
    top-level keys in this order. ``operations`` holds one array per
    requested range, sorted lexicographically by the three key fields,
    each shaped
    ``[product, batch_min, batch_max, base, actions, receipt, ready,
    target]``:

    - ``base`` and ``target`` are the corresponding range query results
      recursively converted to JSON arrays, or ``null`` for an absent
      product;
    - ``actions`` turns the symmetric version difference into an
      ascending-``batch`` sequence: ``["add", A-record]`` for batches
      present only in ``A``, ``["remove", B-record]`` for batches present
      only in ``B`` and ``["replace", B-record, A-record]`` for batches
      present on both sides whose records differ;
    - ``receipt`` is ``[B-receipt, A-receipt]`` and ``ready`` is
      ``[B-ready, A-ready]``, each side ``null`` when the product is
      absent on that side.

    Replaying ``actions`` over ``base`` (inserting added records,
    deleting removed ones and replacing modified ones, then recomputing
    the maximal gap intervals inside the range) and migrating the
    ``receipt``/``ready`` pair yields exactly ``target``. The top-level
    ``changed`` flag is true exactly when at least one range's ``base``
    and ``target`` differ (an empty ``ranges`` being ``false``).
    ...
    for product, batch_min, batch_max in sorted(
            ranges, key=lambda item: (item[0], item[1], item[2])):
        base = _delivery_range_result(
            before_products, product, batch_min, batch_max)
        target = _delivery_range_result(
            after_products, product, batch_min, batch_max)
        if base != target:
            changed = True
        ...
        actions = []
        for batch in sorted(set(before_versions) | set(after_versions)):
            before_record = before_versions.get(batch)
            after_record = after_versions.get(batch)
            if before_record is None:
                actions.append(["add", list(after_record)])
            elif after_record is None:
                actions.append(["remove", list(before_record)])
            elif before_record != after_record:
                actions.append(["replace", list(before_record),
                                list(after_record)])

        receipt_pair = [
            None if base is None
            else _ranges_result_to_json(base[2]),
            None if target is None
            else _ranges_result_to_json(target[2]),
        ]
        ready_pair = [
...
def _decode_commit_log(log: str) -> list:
    """Parse and fully recompute a canonical :func:`delivery_log` document.

    Returns one tuple ``(id, parent, snapshot, target, plan, result_snapshot)``
    per commit in log order, with the four documents kept as their raw
    canonical text spans and ``parent`` ``None`` for the first commit.
    Identifiers, parent linkage, snapshot linkage and every stored result are
    validated by recomputing the whole log with :func:`delivery_log`.

    :raises ValueError: the document is malformed, not canonical, breaks the
        chain, or a recomputed result does not match.
    """
    try:
    ...
    for entry, result_node in zip(entries, result_nodes):
        ...
        result_snapshot = log[snapshot_node[1]:snapshot_node[2]]
        decoded.append((entry[0], entry[1], entry[2], entry[3], entry[4],
                        result_snapshot))
    return decoded
...
def read_commit(log: str, id: str, before: bool = False) -> str:
    ...
    for commit_id, _parent, snapshot, _target, _plan, result_snapshot \
            in decoded:
        if commit_id == id:
...
def plan_delivery_checkout(log: str, source: str, target: str) -> str:
    """Plan a checkout between two commits of a :func:`delivery_log` chain.

    ``log`` must be a ``str`` byte-for-byte matching the canonical output
    of :func:`delivery_log`; the whole chain is recomputed and validated
    (identifiers, parent linkage, snapshot linkage and every stored
    ``result`` against :func:`commit_delivery_updates`). ``source`` and
    ``target`` must each be a ``str`` matching ``[A-Za-z0-9._-]+`` naming
    a commit in the log, and the two commits must lie on one parent chain
    (one must be an ancestor of the other, a commit being its own
    ancestor).

    Returns the canonical compact JSON document
    ``{"source":...,"target":...,"direction":...,"steps":[...],
    "snapshot":...}`` with exactly these five top-level keys in that
    order. ``direction`` is ``"forward"`` when ``source`` is an ancestor
    of ``target`` but differs from it, ``"rollback"`` when ``target`` is
    an ancestor of ``source`` but differs from it, and ``"none"`` when
    ``source`` and ``target`` are the same commit.

    For a forward checkout the ``steps`` list collects, in log order,
    every commit strictly after ``source`` up to and including
    ``target``; for a rollback it collects, in reverse log order, every
    commit from ``source`` down to and including the commit strictly
    after ``target``. Each step is strictly
    ``[id, action, before, after]``: ``action`` repeats ``direction``,
    and ``before``/``after`` embed canonical snapshot objects -- the
    commit's input ``snapshot`` and its ``result.snapshot`` for a forward
    step, swapped for a rollback step (the result snapshot first, the
    input snapshot second).

    The final ``snapshot`` embeds ``target``'s ``result.snapshot``. A
    same-commit checkout carries an empty ``steps`` list. The output
    uses ``ensure_ascii=False``, no whitespace and no trailing newline;
    the inputs are never modified and repeated calls return a
    byte-identical document.

    :raises TypeError: ``log``, ``source`` or ``target`` is not a ``str``.
    :raises ValueError: an identifier is invalid or names no commit, the
        log document, its chain or a recomputed result does not match, or
        the two commits share no ancestor relationship.
    """
    if not isinstance(log, str):
        raise TypeError("log must be a str")
...
def _build_checkout_plan(decoded: list, source: str,
    ...
    if source_pos == target_pos:
        ...
        direction = "forward"
        selected = decoded[source_pos + 1:target_pos + 1]
    else:
        direction = "rollback"
        selected = list(reversed(decoded[target_pos + 1:source_pos + 1]))
    parts = ["{", _json_string("source"), ":", _json_string(source),
             ",", _json_string("target"), ":", _json_string(target),
             ',"direction":', _json_string(direction), ',"steps":[']
    for step_index, (commit_id, _parent, snapshot, _target_doc, _plan,
                    result_snapshot) in enumerate(selected):
        if step_index:
            parts.append(",")
        parts.append("[" + _json_string(commit_id) + ","
                     + _json_string(direction) + ",")
        if direction == "rollback":
            parts.append(result_snapshot + "," + snapshot)
        else:
            parts.append(snapshot + "," + result_snapshot)
        parts.append("]")
...
def _checkout_range_row_text(row: tuple) -> str:
    """Format one ``ranges`` row of a checkout merge/checkpoint document."""
    first_id, last_id, batch_count, before, after = row
    return ("[" + _json_string(first_id) + "," + _json_string(last_id)
            + "," + str(batch_count) + "," + before + "," + after + "]")


def _checkout_audit_row_text(row: tuple) -> str:
    """Format one ``[id, status]`` audit row of a checkout document."""
    batch_id, status = row
    return "[" + _json_string(batch_id) + "," + _json_string(status) + "]"


def _decode_checkout_checkpoint(text: str) -> tuple:
    """Parse and validate a canonical checkout checkpoint document.

    Accepts the canonical three-key output of :func:`merge_checkout_logs`
    and the canonical five-key output of
    :func:`update_checkout_checkpoint`. Returns ``(ranges, audit_rows,
    snapshot)`` where ``ranges`` is a list of ``(first_id, last_id,
    batch_count, before, after)`` tuples with ``before``/``after`` kept as
    raw snapshot documents, ``audit_rows`` a list of ``(id, status)``
    tuples and ``snapshot`` ``None`` or the raw snapshot document.

    :raises ValueError: the document is malformed or not its canonical
        encoding, the ranges do not segment the audit rows, the range
        chain or the terminal snapshot is contradictory, a batch id
        repeats, or the added rows are not suffixes of the accumulated
        rows.
    """
    try:
        node = _parse_json_node(text, _skip_json_ws(text, 0))
    ...
    if not isinstance(top, dict) or list(top) not in (
            ...
            ["ranges", "audit", "snapshot", "added_ranges", "added_audit"]):
    ...
    range_nodes = top["ranges"][0]
    if not isinstance(range_nodes, list):
        raise ValueError('"ranges" must be an array')
    ranges = []
    for row_node in range_nodes:
        row = row_node[0]
        ...
        if not isinstance(first_id, str) or not isinstance(last_id, str):
            raise ValueError("range first and last ids must be str")
        if isinstance(batch_count, bool) \
                or not isinstance(batch_count, int) or batch_count < 1:
            raise ValueError("range batch count must be a positive integer")
        if not isinstance(row[3][0], dict) \
                or not isinstance(row[4][0], dict):
            raise ValueError("range before and after must be JSON objects")
        before = text[row[3][1]:row[3][2]]
        after = text[row[4][1]:row[4][2]]
        _decode_delivery_snapshot(before)
        _decode_delivery_snapshot(after)
        ranges.append((first_id, last_id, batch_count, before, after))
    ...
    for row_node in audit_nodes:
        ...
        if not isinstance(batch_id, str) or not isinstance(status, str):
            raise ValueError("audit id and status must be str")
        if _COMMIT_ID_RE.fullmatch(batch_id) is None:
            raise ValueError(f"invalid batch id {batch_id!r}")
    ...
    position = 0
    previous_after = None
    for first_id, last_id, batch_count, before, after in ranges:
        segment = audit_rows[position:position + batch_count]
        if len(segment) != batch_count \
                or segment[0][0] != first_id or segment[-1][0] != last_id:
            raise ValueError("the ranges do not segment the audit rows")
        if previous_after is not None and before != previous_after:
            raise ValueError("a range's before does not equal the previous "
                             "range's after")
        previous_after = after
        position += batch_count
    if position != len(audit_rows):
        raise ValueError("the ranges do not segment the audit rows")

    snapshot_node = top["snapshot"]
    if snapshot_node[0] is None:
        if ranges:
            raise ValueError("snapshot must equal the last range's after")
        snapshot = None
    elif isinstance(snapshot_node[0], dict):
        snapshot = text[snapshot_node[1]:snapshot_node[2]]
        _decode_delivery_snapshot(snapshot)
        if not ranges or snapshot != ranges[-1][4]:
            raise ValueError("snapshot must equal the last range's after")
    else:
    ...
    added_audit_texts = []
    if has_added:
        added_range_nodes = top["added_ranges"][0]
        added_audit_nodes = top["added_audit"][0]
        if not isinstance(added_range_nodes, list) \
                or not isinstance(added_audit_nodes, list):
            raise ValueError('"added_ranges" and "added_audit" must be '
                             "arrays")
        if len(added_range_nodes) > len(range_nodes) \
                or len(added_audit_nodes) > len(audit_nodes):
            raise ValueError("the added rows must be suffixes of the "
                             "accumulated rows")
        range_offset = len(range_nodes) - len(added_range_nodes)
        for index, added_node in enumerate(added_range_nodes):
            added_text = text[added_node[1]:added_node[2]]
            peer = range_nodes[range_offset + index]
            if added_text != text[peer[1]:peer[2]]:
                raise ValueError('"added_ranges" must be a suffix of '
                                 '"ranges"')
            added_range_texts.append(added_text)
        audit_offset = len(audit_nodes) - len(added_audit_nodes)
        for index, added_node in enumerate(added_audit_nodes):
            added_text = text[added_node[1]:added_node[2]]
            peer = audit_nodes[audit_offset + index]
            if added_text != text[peer[1]:peer[2]]:
                raise ValueError('"added_audit" must be a suffix of "audit"')
            added_audit_texts.append(added_text)
        if sum(row[2] for row in ranges[range_offset:]) \
                != len(added_audit_texts):
            raise ValueError('"added_audit" must hold exactly the batches '
                             'of the "added_ranges"')

    parts = ['{"ranges":[']
    parts.append(",".join(_checkout_range_row_text(row) for row in ranges))
    parts.append('],"audit":[')
    ...
    if has_added:
        ...
        parts.append(",".join(added_range_texts))
        parts.append('],"added_audit":[')
        parts.append(",".join(added_audit_texts))
        parts.append(']')
    parts.append('}')
    if "".join(parts) != text:
        raise ValueError("checkpoint is not its canonical encoding")
    return ranges, audit_rows, snapshot


def update_checkout_checkpoint(log: str, checkpoint, ledgers: tuple) -> str:
    """Fold a batch of :func:`checkout_log` ledgers into a checkpoint.
    ...
    previous output of this function; a previous output's ``added_ranges``
    and ``added_audit`` are validated to be suffixes of the accumulated
    ``ranges`` and ``audit``. ``ledgers`` must be a ``tuple`` whose items
    are each a ``str`` byte-for-byte matching the canonical output of
    :func:`checkout_log`; every ledger is re-verified with
    :func:`replay_checkout` against ``log`` before it contributes.

    An empty ledger (one with no batches) produces no range and leaves
    the snapshot boundary untouched; the non-empty ledgers append to the
    accumulated state in input order. The batch ids of the non-empty
    ledgers must be globally unique, also against the accumulated audit
    rows, and the first batch's ``before`` of the first non-empty ledger
    must byte-for-byte equal the checkpoint's ``snapshot`` (with no
    checkpoint value the accumulation starts from ``[]``, ``[]`` and
    ``null`` and the first non-empty ledger is unconstrained), exactly as
    the non-empty ledgers must chain among themselves in
    :func:`merge_checkout_logs`.
    ...
    :func:`merge_checkout_logs` structure and hold the accumulated
    result: ``ranges`` rows are ``[first_id, last_id, batch_count,
    before, after]`` with ``before``/``after`` embedded as snapshot
    objects, ``audit`` lists one ``[id, status]`` row per batch in global
    batch order and ``snapshot`` is ``null`` when nothing has accumulated
    and otherwise the last non-empty ledger's ``snapshot``.
    ``added_ranges`` and ``added_audit`` hold only the rows this call
    newly appended and are therefore suffixes of the accumulated arrays;
    when nothing is added both are ``[]`` and the accumulated values are
    carried over unchanged. The output uses ``ensure_ascii=False``, no
    ...
    :raises ValueError: a document is not its canonical encoding, the
        checkpoint's ranges contradict its audit segmentation or its
        terminal snapshot, a previous output's added rows are not
        suffixes of the accumulated rows, a batch id repeats, the first
        non-empty ledger's first ``before`` does not equal the
    ...
    seen = {batch_id for batch_id, _ in old_audit}
    new_ranges = []
    new_audit = []
    ...
    for ledger in ledgers:
        ...
        for row_node in batches:
            ...
            if batch_id in seen:
                raise ValueError(f"duplicate batch id {batch_id!r} across "
                                 "the checkpoint and the ledgers")
            seen.add(batch_id)
            if first_id is None:
                first_id = batch_id
                first_before = ledger[row[1][1]:row[1][2]]
        ...
        if previous_snapshot is not None \
                and first_before != previous_snapshot:
            raise ValueError(f"ledger starting at batch {first_id!r} must "
                             "begin from the previous snapshot")
        new_ranges.append((first_id, last_id, len(batches), first_before,
                           ledger_snapshot))
        previous_snapshot = ledger_snapshot
        snapshot = ledger_snapshot
    all_ranges = old_ranges + new_ranges
    all_audit = old_audit + new_audit
    parts = ['{"ranges":[']
    parts.append(",".join(_checkout_range_row_text(row)
...
def _checkout_recovery_units(ranges: list, audit_rows: list) -> list:
    """Group checkpoint rows into ``(range, audit_segment)`` units.

    Each unit pairs one ``ranges`` row with the ``audit`` segment of
    ``batch_count`` rows it covers, in row order.
    """
    units = []
    position = 0
    for row in ranges:
        batch_count = row[2]
        units.append((row, tuple(audit_rows[position:position + batch_count])))
        position += batch_count
    return units


def _format_checkout_recovery_units(units: list) -> str:
    """Format a ``missing``/``pending`` array of ``[range, audit]`` rows."""
    rows = []
    for range_row, segment in units:
        rows.append("[" + _checkout_range_row_text(range_row) + ",["
...
def plan_checkout_recovery(log: str, checkpoint, ledgers: tuple) -> str:
    """Plan how a checkpoint can recover from a replayed ledger batch.
    ...
    ``str`` byte-for-byte matching the canonical output of
    :func:`merge_checkout_logs`, or a ``str`` byte-for-byte matching a
    previous output of :func:`update_checkout_checkpoint`. ``ledgers``
    ...
    An empty ledger (one with no batches) produces no range and leaves
    the snapshot boundary untouched. The non-empty ledgers are merged
    into a candidate checkpoint exactly as in
    :func:`merge_checkout_logs`: their batch ids must be globally unique
    across the batch and the first batch's ``before`` of every non-empty
    ledger after the first must byte-for-byte equal the previous
    non-empty ledger's ``snapshot``.

    The candidate and the old checkpoint are then compared in units of
    one ``ranges`` row plus the ``audit`` segment of ``batch_count`` rows
    it covers, and the longest common prefix of units is determined. If
    both sides still hold units after the prefix, the histories have
    forked and the recovery is rejected. The old checkpoint's remaining
    units are the ``missing`` rows to re-apply and the candidate's
    remaining units are the ``pending`` rows to commit; one of the two
    is always empty.
    ...
    ``missing`` and ``pending`` are arrays of ``[range, audit]`` pairs
    with ``range`` a five-item ``[first_id, last_id, batch_count, before,
    after]`` row (``before``/``after`` embedded as snapshot objects) and
    ``audit`` the segment's ``[id, status]`` rows. ``snapshot`` is the
    candidate's terminal snapshot object when ``pending`` is non-empty
    and otherwise the old checkpoint's terminal snapshot object, being
    ``null`` when both are empty. The output uses
    ``ensure_ascii=False``, no whitespace and no trailing newline, and
    repeated calls return a byte-identical document.
    ...
    :raises ValueError: a document is not its canonical encoding, a
        ...
        ``snapshot``, or the candidate and the old checkpoint fork after
        their common prefix.
    """
    if not isinstance(log, str):
        raise TypeError("log must be a str")
    ...
    for ledger in ledgers:
        if not isinstance(ledger, str):
            raise TypeError("each ledger must be a str")
    candidate = merge_checkout_logs(log, ledgers)
    new_ranges, new_audit, new_snapshot = \
        _decode_checkout_checkpoint(candidate)
    if checkpoint is None:
        old_ranges, old_audit, old_snapshot = [], [], None
    else:
        old_ranges, old_audit, old_snapshot = \
            _decode_checkout_checkpoint(checkpoint)
    old_units = _checkout_recovery_units(old_ranges, old_audit)
    new_units = _checkout_recovery_units(new_ranges, new_audit)
    common = 0
    ...
    if common < len(old_units) and common < len(new_units):
        raise ValueError("the checkpoint and the ledgers fork after "
                         f"{common} common range(s)")
    missing = old_units[common:]
    pending = new_units[common:]
    snapshot = new_snapshot if pending else old_snapshot
...
def _decode_recovery_receipt(text: str, direction: str, start_snapshot):
    """Parse and validate a canonical :func:`build_recovery_receipt` receipt.

    ``direction`` is the plan's active recovery arm (``"pending"``,
    ``"missing"`` or ``"none"``) and ``start_snapshot`` is ``None`` or the
    raw snapshot document the receipt's ``start`` must equal.

    Returns ``(segments, end_snapshot, complete)`` with ``segments`` a list
    of ``(origin, target, batches)`` triples (``batches`` a tuple of
    ``(id, status)`` rows in document order), ``end_snapshot`` ``None`` or
    the raw snapshot document, and ``complete`` the receipt's boolean.

    :raises ValueError: the JSON or shape is bad, a value violates the
        receipt contract, the direction or start does not match the plan,
    ...
    if not isinstance(top, dict) or list(top) != [
        ...
        raise ValueError("receipt must be a JSON object with exactly the keys "
                         '"direction", "start", "end", "segments" and '
                         '"complete"')
    receipt_direction = top["direction"][0]
    if receipt_direction not in ("pending", "missing", "none"):
        raise ValueError('receipt "direction" must be "pending", "missing" or '
                         '"none"')
    if receipt_direction != direction:
    ...
    def _raw_snapshot(key):
        ...
        if isinstance(snapshot_node[0], dict):
            ...
            return raw
        raise ValueError(f'receipt "{key}" must be null or a JSON object')

    receipt_start = _raw_snapshot("start")
    if receipt_start != start_snapshot:
        raise ValueError('receipt "start" does not match the plan')
    receipt_end = _raw_snapshot("end")

    receipt_nodes = top["segments"][0]
    if not isinstance(receipt_nodes, list):
        raise ValueError('receipt "segments" must be an array')
    segments = []
    seen_ids = set()
    for segment_node in receipt_nodes:
        children = segment_node[0]
        if not isinstance(children, list) or len(children) != 3:
            raise ValueError("each receipt segment must be a three-item array "
                             "[from, to, batches]")
        origin = children[0][0]
        target = children[1][0]
        if isinstance(origin, bool) or not isinstance(origin, int) \
                or origin < 0:
            raise ValueError("receipt segment origin must be a non-negative "
                             "integer")
        if isinstance(target, bool) or not isinstance(target, int) \
                or target < 0:
            raise ValueError("receipt segment target must be a non-negative "
                             "integer")
        if origin >= target:
            raise ValueError("receipt segment must confirm at least one unit")
        if segments and origin != segments[-1][1]:
            raise ValueError("receipt segments must chain without a gap")
        batch_nodes = children[2][0]
        if not isinstance(batch_nodes, list):
            raise ValueError("receipt segment batches must be an array")
        batches = []
        for row_node in batch_nodes:
            row = row_node[0]
            if not isinstance(row, list) or len(row) != 2:
    ...
    parts = ['{"direction":', _json_string(receipt_direction), ',"start":',
             "null" if receipt_start is None else receipt_start, ',"end":',
             "null" if receipt_end is None else receipt_end, ',"segments":[']
    segment_text = []
    for origin, target, batches in segments:
        segment_text.append("[" + str(origin) + "," + str(target) + ",["
                            + ",".join(_checkout_audit_row_text(row)
    ...
    return segments, receipt_end, complete


def merge_recovery_receipts(plan: str, receipts: tuple) -> str:
    """Merge canonical recovery receipts of one
    :func:`plan_checkout_recovery` plan into a single receipt.

    ``plan`` must be a ``str`` byte-for-byte matching the canonical output
    of :func:`plan_checkout_recovery`. ``receipts`` must be a ``tuple``
    whose items are each a ``str`` byte-for-byte matching the canonical
    output of :func:`build_recovery_receipt` for that same plan.

    Every receipt's ``direction`` and ``start`` must match the plan. The
    receipts are read in tuple order: each receipt's confirmed unit count
    (the ``to`` of its last segment, zero when it has no segments) must
    strictly increase over the previous receipt, its segments must chain
    without gaps from the previous receipt's target, and the previous
    receipt's whole segment sequence must be a prefix of this receipt's
    segments (identical batch rows included), so the earlier receipts are
    a complete prefix of the later ones. Each newly covered segment's
    batches must equal the plan units' audit rows for that range, no
    batch id may repeat across the merge, the first newly covered unit's
    ``before`` must equal the previous receipt's ``end``, each receipt's
    ``end`` must equal its last newly covered unit's ``after`` (or
    ``start`` when it covers nothing), and its ``complete`` flag must
    agree with the confirmed prefix. An empty ``receipts`` tuple is
    valid.
    ...
    units). ``start`` is the first unit's ``before`` snapshot and
    otherwise the plan's ``snapshot`` (possibly ``null``). ``end`` is the
    last receipt's ``end`` embedded byte for byte, or ``start`` when
    ``receipts`` is empty. ``receipts`` embeds the input receipts in
    tuple order as their original canonical objects without re-encoding
    their snapshots. ``complete`` is the last receipt's ``complete``
    value, and is true only when the plan has no units when ``receipts``
    is empty. The output uses ``ensure_ascii=False``, no whitespace and no
    trailing newline; the inputs are never modified and repeated calls
    return a byte-identical document.

    :raises TypeError: ``plan`` or a receipt is not a ``str``, or
        ``receipts`` is not a ``tuple``.
    :raises ValueError: the plan is not the canonical plan encoding or is
        forked (both arms non-empty), a receipt is malformed or not its
        canonical encoding, a receipt's ``direction`` or ``start`` does
        not match the plan, the confirmed counts do not strictly
        increase, the segments gap or fail the prefix relationship, the
        batch rows disagree with the plan, a batch id repeats, or an
        ``end`` or ``complete`` value does not match the confirmed
        prefix.
    """
    if not isinstance(plan, str):
        raise TypeError("plan must be a str")
    ...
    for receipt in receipts:
        ...
        decoded.append(_decode_recovery_receipt(
            receipt, direction, start_snapshot))
    ...
    for index, (segments, end_snapshot, receipt_complete) in enumerate(
        ...
        if len(segments) < len(previous_segments) \
            ...
            raise ValueError("each receipt's segments must extend the "
                             "previous receipt's segments")
        if index > 0 and target <= confirmed:
            raise ValueError("receipt confirmed counts must strictly "
                             "increase")
        if target > len(units):
            raise ValueError("a receipt confirms more units than the plan "
                             "holds")
        first_new = len(previous_segments)
        if segments:
            if segments[first_new][0] != confirmed:
                raise ValueError("the receipt segments do not continue the "
                                 "confirmed prefix")
            if units[confirmed][0][3] != previous_end:
                raise ValueError("the receipt does not continue at the "
        ...
        for origin, segment_target, batches in segments[first_new:]:
            ...
            if tuple(expected) != batches:
                raise ValueError("a receipt segment's batches do not match "
            ...
            for batch_id, _status in batches:
                ...
                seen_ids.add(batch_id)
        if target:
            expected_end = units[target - 1][0][4]
        else:
            expected_end = start_snapshot
        if end_snapshot != expected_end:
            raise ValueError('receipt "end" does not match the confirmed '
                             "prefix")
        if receipt_complete != (target == len(units)):
            raise ValueError('receipt "complete" does not match the '
                             "confirmed prefix")
        confirmed = target
    ...
    if receipts:
        end_snapshot = decoded[-1][1]
        complete = decoded[-1][2]
    else:
        end_snapshot = start_snapshot
        complete = not units
    ...
    parts = ['{"direction":', _json_string(direction), ',"start":',
             ...
             "null" if end_snapshot is None else end_snapshot, ',"receipts":[',
             ",".join(receipts), '],"complete":',
             "true" if complete else "false", "}"]
    return "".join(parts)


def _decode_recovery_history_summary(text: str, direction: str,
                                     start_snapshot) -> tuple:
    """Validate a canonical :func:`merge_recovery_receipts` summary.

    ``direction`` is the plan's active recovery arm (``"pending"``,
    ``"missing"`` or ``"none"``) and ``start_snapshot`` is ``None`` or the
    raw snapshot document the summary's ``start`` must equal.

    Returns ``(receipt_texts, receipts, end_snapshot, complete)`` with
    ``receipt_texts`` the raw canonical embedded receipt documents,
    ``receipts`` a tuple of :func:`_decode_recovery_receipt` results,
    ``end_snapshot`` ``None`` or the raw ``end`` document, and
    ``complete`` the summary's boolean.

    :raises ValueError: the JSON or shape is bad, the direction or start
        does not match the plan, an embedded receipt is not its canonical
        encoding, or the text is not its canonical summary encoding.
    """
    try:
        node = _parse_json_node(text, _skip_json_ws(text, 0))
    except ValueError as exc:
    ...
    summary_start = _raw_snapshot("start")
    if summary_start != start_snapshot:
    ...
    for receipt_node in receipt_nodes:
        ...
        receipts.append(_decode_recovery_receipt(
            raw, direction, start_snapshot))
        receipt_texts.append(raw)
    ...
    parts = ['{"direction":', _json_string(summary_direction), ',"start":',
             "null" if summary_start is None else summary_start, ',"end":',
             "null" if summary_end is None else summary_end, ',"receipts":[',
             ",".join(receipt_texts), '],"complete":',
             "true" if complete else "false", "}"]
    if "".join(parts) != text:
        raise ValueError("summary is not its canonical encoding")
    return tuple(receipt_texts), tuple(receipts), summary_end, complete


def build_recovery_history(plan: str, summaries: tuple) -> str:
    """Build a history across :func:`merge_recovery_receipts` summaries.
    ...
    Every summary's ``direction`` and ``start`` must agree with the plan.
    The summaries are read in tuple order: each summary's embedded
    receipts must chain against the plan exactly as in
    :func:`merge_recovery_receipts`, and after the first summary each
    summary's ``receipts`` array must be a strict extension of the previous
    summary's array (identical embedded receipts included), so the
    confirmed unit count keeps strictly increasing with no batch id
    repeated across newly covered units. Nothing may be appended after the
    ...
    ``resume`` and ``complete`` in that order. ``direction`` is the plan's
    active arm (``"pending"``, ``"missing"`` or ``"none"``) and ``start``
    is the first unit's ``before`` snapshot, otherwise the plan's
    ``snapshot`` (possibly ``null``). ``end`` is the last summary's ``end``
    embedded byte for byte, or ``start`` when ``summaries`` is empty.
    ``runs`` follows the summaries in order; each is
    ``[confirmed, end, complete]`` with ``confirmed`` the cumulative number
    of confirmed units at that summary. ``audit`` collects the newly
    covered ``[id, status]`` rows in plan order. ``resume`` embeds the
    canonical :func:`execute_checkout_recovery` state object at the
    confirmed prefix when work remains and is ``null`` once complete.
    ``complete`` is the last summary's ``complete`` value, and is true only
    when the plan has no units when ``summaries`` is empty. The output uses
    ``ensure_ascii=False``, no whitespace and no trailing newline; the
    inputs are never modified and repeated calls return a byte-identical
    document.
    ...
    :raises ValueError: the plan is not the canonical plan encoding or is
        ...
        the previous summary's, the embedded receipts do not chain
        against the plan, a batch id repeats, or an ``end`` or
        ``complete`` value does not match the confirmed prefix.
    """
    if not isinstance(plan, str):
        raise TypeError("plan must be a str")
    ...
    for summary_index, summary in enumerate(summaries):
        ...
        receipt_texts, receipts, summary_end, summary_complete = \
            _decode_recovery_history_summary(
                summary, direction, start_snapshot)
        if merge_recovery_receipts(plan, receipt_texts) != summary:
            raise ValueError("summary is not the canonical merge of its "
                             "receipts for this plan")
        if summary_index > 0 and (
                len(receipts) <= len(previous_receipts)
                or receipts[:len(previous_receipts)] != previous_receipts):
            raise ValueError("each summary's receipts must strictly extend "
                             "the previous summary's receipts")
        target = confirmed
        skip_segments = (len(previous_receipts[-1][0])
                         if previous_receipts else 0)
        for segments, _receipt_end, _receipt_complete in \
                receipts[len(previous_receipts):]:
            for origin, segment_target, batches in \
                ...
                for batch_id, _status in batches:
                    if batch_id in seen_ids:
                        raise ValueError(f"duplicate batch id {batch_id!r}")
                    seen_ids.add(batch_id)
                audit_rows.extend(batches)
            target = segments[-1][1] if segments else 0
            confirmed = target
            skip_segments = len(segments)
    ...
    if complete:
        ...
        resume = _format_checkout_recovery_state(
...
def _decode_recovery_history(text: str, common: int, direction: str,
                             start_snapshot, units: list,
                             plan_snapshot) -> tuple:
    """Validate a canonical :func:`build_recovery_history` document.

    The history must describe the recovery plan identified by ``common``,
    ``direction``, ``start_snapshot`` and ``units`` (with
    ``plan_snapshot`` ``None`` or the plan's raw terminal snapshot).

    Returns ``(runs, audit_rows, end_snapshot, resume_state)`` with
    ``runs`` a tuple of ``(confirmed, end, complete)`` triples (``end``
    ``None`` or the raw embedded snapshot), ``audit_rows`` a tuple of
    ``(id, status)`` rows in document order, ``end_snapshot`` ``None`` or
    the raw ``end`` document, and ``resume_state`` ``None`` when complete
    or otherwise the raw embedded canonical
    :func:`execute_checkout_recovery` state document.

    :raises ValueError: the JSON or shape is bad, a value violates the
        history contract, the direction or start does not match the plan,
        the runs or audit rows do not follow the plan, a batch id
        repeats, or the text is not its canonical encoding.
    """
    try:
        node = _parse_json_node(text, _skip_json_ws(text, 0))
    except ValueError as exc:
    ...
    if not isinstance(top, dict) or list(top) != [
            ...
            "complete"]:
        raise ValueError("history must be a JSON object with exactly the "
                         'keys "direction", "start", "end", "runs", '
                         '"audit", "resume" and "complete"')
    history_direction = top["direction"][0]
    if history_direction != direction:
    ...
    history_start = _raw_snapshot("start")
    if history_start != start_snapshot:
        raise ValueError('history "start" does not match the plan')
    history_end = _raw_snapshot("end")

    run_nodes = top["runs"][0]
    if not isinstance(run_nodes, list):
        raise ValueError('history "runs" must be an array')
    runs = []
    previous_target = 0
    target = 0
    for run_node in run_nodes:
        ...
        if isinstance(confirmed, bool) or not isinstance(confirmed, int) \
                or confirmed < 0 or confirmed > len(units) \
                or (runs and confirmed <= previous_target):
            raise ValueError("history run confirmed count must strictly "
                             "advance between zero and the number of plan "
                             "units")
        run_end_node = children[1]
        if run_end_node[0] is None:
            run_end = None
        elif isinstance(run_end_node[0], dict):
            run_end = text[run_end_node[1]:run_end_node[2]]
            _decode_delivery_snapshot(run_end)
        else:
            raise ValueError("history run end must be null or a JSON object")
        run_complete = children[2][0]
        if not isinstance(run_complete, bool):
            raise ValueError("history run complete must be a boolean")
        if run_end != (units[confirmed - 1][0][4] if confirmed
                       else start_snapshot):
            raise ValueError('history run "end" does not match its confirmed '
                             "prefix")
        if run_complete != (confirmed == len(units)):
            raise ValueError('history run "complete" does not match its '
                             "confirmed prefix")
        runs.append((confirmed, run_end, run_complete))
        target = confirmed
        previous_target = confirmed

    audit_node = top["audit"][0]
    if not isinstance(audit_node, list):
        raise ValueError('history "audit" must be an array')
    expected_audit = []
    for unit in units[:target]:
        expected_audit.extend(unit[1])
    audit_rows = []
    seen_ids = set()
    for row_node in audit_node:
        row = row_node[0]
        if not isinstance(row, list) or len(row) != 2:
            raise ValueError("each history audit row must be a two-item "
                             "array [id, status]")
        batch_id = row[0][0]
        status = row[1][0]
        if not isinstance(batch_id, str) or not isinstance(status, str):
        ...
        seen_ids.add(batch_id)
        audit_rows.append((batch_id, status))
    if tuple(audit_rows) != tuple(expected_audit):
        raise ValueError('history "audit" must list the confirmed plan '
                         "units' batch rows in order")

    resume_node = top["resume"]
    if resume_node[0] is None:
        resume_state = None
    elif isinstance(resume_node[0], dict):
        resume_state = text[resume_node[1]:resume_node[2]]
    else:
        raise ValueError('history "resume" must be null or a JSON object')
    complete = target == len(units)
    if bool(resume_state is None) != complete:
        raise ValueError('history "resume" must be null exactly when the '
                         "history is complete")
    if not complete:
        records = units[:target]
        if target:
            resume_snapshot = units[target - 1][0][4]
        elif units:
            resume_snapshot = units[0][0][3]
        else:
            resume_snapshot = plan_snapshot
        _decode_checkout_recovery_state(
            resume_state, common, direction, units, plan_snapshot)
        expected_resume = _format_checkout_recovery_state(
            common, direction, target, records, resume_snapshot, False)
        if resume_state != expected_resume:
            raise ValueError('history "resume" must be the canonical '
                             "execute_checkout_recovery state at the "
                             "confirmed prefix")
    expected_end = runs[-1][1] if runs else start_snapshot
    if history_end != expected_end:
        raise ValueError('history "end" does not match the last run')
    history_complete = top["complete"][0]
    if not isinstance(history_complete, bool) \
            or history_complete != complete:
        raise ValueError('history "complete" does not match the confirmed '
                         "prefix")

    parts = ['{"direction":', _json_string(direction), ',"start":',
             "null" if start_snapshot is None else start_snapshot, ',"end":',
             "null" if history_end is None else history_end, ',"runs":[']
    run_text = []
    for run_confirmed, run_end, run_complete in runs:
        run_text.append("[" + str(run_confirmed) + ","
                        + ("null" if run_end is None else run_end) + ","
    ...
    parts.append("}")
    if "".join(parts) != text:
        raise ValueError("history is not its canonical encoding")
    return tuple(runs), tuple(audit_rows), history_end, resume_state


def _recovery_plan_view(plan_text: str) -> tuple:
    """Decode a recovery plan into its active-arm validation view.

    Returns ``(common, direction, units, start_snapshot, plan_snapshot)``
    with ``direction`` ``"pending"``, ``"missing"`` or ``"none"`` and
    ``start_snapshot`` the first active unit's ``before`` snapshot,
    otherwise the plan's terminal snapshot (possibly ``None``).
    """
    common, missing, pending, plan_snapshot = \
        _decode_checkout_recovery_plan(plan_text)
    if missing and pending:
        raise ValueError("the plan forks: both missing and pending units "
                         "remain")
    if pending:
    ...
    return common, direction, units, start_snapshot, plan_snapshot


def _history_node_text(text: str, key: str) -> str:
    """Return the raw embedded object text for one history top-level key."""
    node = _parse_json_node(text, _skip_json_ws(text, 0))
    value_node = node[0][key]
    return text[value_node[1]:value_node[2]]


def _history_audit_rows(text: str) -> tuple:
    """Return a history document's ``(id, status)`` audit rows."""
    node = _parse_json_node(text, _skip_json_ws(text, 0))
    return tuple((row_node[0][0][0], row_node[0][1][0])
                 for row_node in node[0]["audit"][0])


def _history_run_texts(text: str) -> tuple:
    """Return the raw element texts of a history document's ``runs``."""
    node = _parse_json_node(text, _skip_json_ws(text, 0))
    return tuple(text[run_node[1]:run_node[2]]
                 for run_node in node[0]["runs"][0])


def update_recovery_index(index, items: tuple) -> str:
    """Fold ``(plan, history)`` recovery pairs into a recovery index.

    ``index`` is either ``None`` (no entries yet) or a ``str``
    byte-for-byte matching a previous canonical output of this function.
    ``items`` must be a ``tuple`` whose items are each a two-item
    ``(plan, history)`` tuple of ``str`` values; ``plan`` must be a
    canonical :func:`plan_checkout_recovery` document and ``history`` the
    canonical :func:`build_recovery_history` output for that same plan.

    The pairs are validated in tuple order. Every pair except the final
    one must carry a complete history; the final pair may be in progress,
    in which case its ``end`` is the index snapshot and its ``resume``
    state names the next unit to confirm. Consecutive pairs must chain:
    the first active unit's ``before`` snapshot of every plan after the
    first must byte-for-byte equal the previous history's ``end``, and
    batch ids must be unique across every history audit in the index.
    When the previous index ends with an incomplete entry, the first pair
    must use that same plan and its history must keep the entry's
    ``runs`` and ``audit`` prefix while strictly extending both,
    replacing the entry; an already complete entry can never be replaced.
    An empty ``items`` tuple leaves an existing index unchanged.

    Returns the canonical compact JSON document with exactly the four
    top-level keys ``entries``, ``snapshot``, ``resume`` and ``complete``
    in that order. ``entries`` preserves pair order, each entry a
    two-item array embedding the plan object and the history object in
    that order. ``snapshot`` is the last history's ``end`` embedded byte
    for byte, or ``null`` with no entries. ``resume`` is
    ``[position, state]`` with ``position`` the zero-based index of the
    last entry and ``state`` that history's embedded
    :func:`execute_checkout_recovery` resume state when the last history
    is incomplete, and otherwise ``null``. ``complete`` is true only for
    an empty index or when the last entry is complete. The output uses
    ``ensure_ascii=False``, no whitespace and no trailing newline; the
    inputs are never modified and repeated calls return a byte-identical
    document.

    :raises TypeError: ``index`` is neither ``None`` nor a ``str``,
        ``items`` is not a ``tuple``, an item is not a two-item tuple, or
        a plan or history is not a ``str``.
    :raises ValueError: the index, a plan or a history document is
        malformed or not its canonical encoding, a history does not match
        its plan, a non-last history is incomplete, an already complete
        entry would be replaced, an incomplete entry is not replaced by a
        strict extension of the same plan, consecutive pairs do not
        chain, or a batch id repeats globally.
    """
    ...
    if index is not None:
        try:
            node = _parse_json_node(index, _skip_json_ws(index, 0))
        except ValueError as exc:
            raise ValueError(f"index is not a valid JSON document ({exc})") \
                from exc
        if _skip_json_ws(index, node[2]) != len(index):
            raise ValueError("index has trailing data after the JSON "
                             "document")
        top = node[0]
        if not isinstance(top, dict) or list(top) != [
                "entries", "snapshot", "resume", "complete"]:
            raise ValueError("index must be a JSON object with exactly the "
                             'keys "entries", "snapshot", "resume" and '
                             '"complete"')
        ...
        for entry_node in entry_nodes:
            pair = entry_node[0]
            if not isinstance(pair, list) or len(pair) != 2 \
                    or not isinstance(pair[0][0], dict) \
                    or not isinstance(pair[1][0], dict):
                raise ValueError("each index entry must be a [plan, "
                                 "history] pair of JSON objects")
            plan_text = index[pair[0][1]:pair[0][2]]
            history_text = index[pair[1][1]:pair[1][2]]
            _common, _direction, _units, start_snapshot, _plan_snapshot = \
                _recovery_plan_view(plan_text)
            _decode_recovery_history(
                history_text, _common, _direction, start_snapshot, _units,
                _plan_snapshot)
            entries.append([plan_text, history_text])
            stored_starts.append(
                "null" if start_snapshot is None else start_snapshot)
        for _stored_plan, stored_history in entries[:-1]:
            if '"resume":null' not in stored_history:
                raise ValueError("every index entry except the last must "
                                 "hold a complete history")
        for stored_position in range(1, len(entries)):
            previous_end = _history_node_text(
                entries[stored_position - 1][1], "end")
            if stored_starts[stored_position] != previous_end:
                raise ValueError("each index plan after the first must "
                                 "start at the previous history's end")
        snapshot_node = top["snapshot"]
        if snapshot_node[0] is None:
            index_snapshot = None
        elif isinstance(snapshot_node[0], dict):
            index_snapshot = index[snapshot_node[1]:snapshot_node[2]]
            _decode_delivery_snapshot(index_snapshot)
        else:
            raise ValueError('index "snapshot" must be null or a JSON '
                             "object")
        expected_snapshot = _history_node_text(entries[-1][1], "end") \
            if entries else "null"
        if ("null" if index_snapshot is None else index_snapshot) \
                != expected_snapshot:
            raise ValueError('index "snapshot" does not match its last '
                             "entry")
        resume_node = top["resume"]
        if resume_node[0] is None:
            if entries and '"resume":null' not in entries[-1][1]:
                raise ValueError('index "resume" must embed the last '
                                 "history's resume state")
        else:
            if not isinstance(resume_node[0], list) or len(resume_node[0]) != 2:
                raise ValueError('index "resume" must be null or a '
                                 "[position, state] pair")
            children = resume_node[0]
            resume_position = children[0][0]
            if isinstance(resume_position, bool) \
                    or not isinstance(resume_position, int) \
                    or resume_position != len(entries) - 1 \
                    or not isinstance(children[1][0], dict):
                raise ValueError('index "resume" position must name the '
                                 "last entry and embed a state object")
            state_text = index[children[1][1]:children[1][2]]
            if not entries or state_text != _history_node_text(
                    entries[-1][1], "resume"):
                raise ValueError('index "resume" state must equal the last '
                                 "history's resume")
        complete_node = top["complete"][0]
        if not isinstance(complete_node, bool):
            raise ValueError('index "complete" must be a boolean')
        expected_complete = not entries or '"resume":null' in entries[-1][1]
        if complete_node != expected_complete:
            raise ValueError('index "complete" does not match its last '
                             "entry")
        canonical_entries = ",".join(
            "[" + plan_text + "," + history_text + "]"
            for plan_text, history_text in entries)
        canonical = ('{"entries":[' + canonical_entries + '],"snapshot":'
                     + expected_snapshot + ',"resume":')
        if resume_node[0] is None:
            canonical += "null"
        else:
            canonical += index[resume_node[1]:resume_node[2]]
        canonical += ',"complete":' + ("true" if expected_complete else
                                       "false") + "}"
        if canonical != index:
    ...
    for item in items:
        ...
        plan_text, history_text = item
        if not isinstance(plan_text, str) or not isinstance(history_text, str):
            raise TypeError("each item's plan and history must be a str")
        common, direction, units, start_snapshot, plan_snapshot = \
            _recovery_plan_view(plan_text)
        _decode_recovery_history(
            history_text, common, direction, start_snapshot, units,
            plan_snapshot)
        if entries:
            previous_plan, previous_history = entries[-1]
            previous_complete = '"resume":null' in previous_history
            if plan_text == previous_plan:
                if previous_complete:
                    raise ValueError("an already complete index entry cannot "
                                     "be replaced")
            if not previous_complete:
                if plan_text != previous_plan:
                    raise ValueError("an incomplete index entry can only be "
                                     "replaced by a history of the same plan")
                previous_runs = _history_run_texts(previous_history)
                new_runs = _history_run_texts(history_text)
                if len(new_runs) <= len(previous_runs) \
                        or new_runs[:len(previous_runs)] != previous_runs:
                    raise ValueError("the replacement history's runs must "
                                     "strictly extend the entry's runs")
                previous_audit = _history_audit_rows(previous_history)
                new_audit = _history_audit_rows(history_text)
                if len(new_audit) <= len(previous_audit) \
                        or new_audit[:len(previous_audit)] \
                        != previous_audit:
                    raise ValueError("the replacement history's audit must "
                                     "strictly extend the entry's audit")
                entries[-1] = [plan_text, history_text]
                continue
            previous_end = _history_node_text(previous_history, "end")
            later_start = "null" if start_snapshot is None else start_snapshot
            if later_start != previous_end:
                raise ValueError("each plan after the first must start at "
                                 "the previous history's end")
        entries.append([plan_text, history_text])

    for pair in entries[:-1]:
        if '"resume":null' not in pair[1]:
            raise ValueError("every entry except the last must hold a "
                             "complete history")

    seen_ids = set()
    for pair in entries:
        for batch_id, _status in _history_audit_rows(pair[1]):
            if batch_id in seen_ids:
                raise ValueError(f"duplicate batch id {batch_id!r} across "
                                 "the recovery index")
            seen_ids.add(batch_id)
    ...
    if entries:
        last_history = entries[-1][1]
        snapshot_text = _history_node_text(last_history, "end")
        last_complete = '"resume":null' in last_history
        if last_complete:
            resume_text = "null"
        else:
            resume_text = "[" + str(len(entries) - 1) + "," \
                + _history_node_text(last_history, "resume") + "]"
    else:
        snapshot_text = "null"
        resume_text = "null"
        last_complete = True
    ...
    parts = ['{"entries":[']
    parts.append(",".join("[" + plan_text + "," + history_text + "]"
                          for plan_text, history_text in entries))
    parts.append('],"snapshot":')
    ...
    parts.append(',"complete":')
    parts.append("true" if last_complete else "false")
    parts.append("}")
    return "".join(parts)
