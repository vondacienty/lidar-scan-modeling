...
def rollback_delta_assessments(base: tuple, deltas: tuple) -> tuple:
    ...
    with the delta summands accumulated in ascending Decimal order and then
    subtracted, quantized to six decimal places as a float (negative zero
    normalized). Each resulting summary must satisfy ``min_dzmin <=
    max_dzmax`` and ``match_count > 0``; otherwise ``ValueError`` is raised.
    The result does not depend on the order of ``deltas`` and the inputs are
    never modified.
    ...
    with localcontext() as ctx:
        ...
        for level, ix_min, iy_min, ix_max, iy_max, summary in base:
            ...
            if (isinstance(base_min, int)
                    and all(isinstance(term, int) for term in dmin_terms)):
            ...
            if (isinstance(base_max, int)
                    and all(isinstance(term, int) for term in dmax_terms)):
