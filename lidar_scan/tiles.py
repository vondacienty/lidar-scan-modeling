...
def encode_tile_window_stats(pyramid: tuple, level: int,
    ...
    ``tiles``. The first five values echo the arguments; ``tiles`` is the
    array of eleven-value tiles at ``level`` whose closed cell-index
    intervals intersect the window (``tile.ix1 >= ix_min and
    tile.ix0 <= ix_max and tile.iy1 >= iy_min and tile.iy0 <= iy_max``), in
    the level's stored order (``[]`` when nothing matches). Integers are
    decimal; the four statistics use exactly six decimal places (negative
    zero written as ``0.000000``). The output has no whitespace, ASCII is not
    escaped and ``NaN``/``Infinity`` never appear. The input is never
    modified.
