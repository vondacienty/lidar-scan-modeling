...
def query_tile_pyramid_windows(pyramid: tuple, windows: tuple) -> tuple:
    ...
    For each window, a tile at that level matches when its closed cell-index
    intervals intersect the window: ``tile.ix1 >= ix_min and tile.ix0 <=
    ix_max and tile.iy1 >= iy_min and tile.iy0 <= iy_max``.
    ...
    :raises ValueError: the pyramid's structure, ordering, duplicates, fields
        or cell bounds are bad, ``level`` is out of range or the window
        bounds are inverted.
    """
    if not isinstance(pyramid, tuple):
        raise TypeError("pyramid must be a tuple")
