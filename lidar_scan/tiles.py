...
def apply_tile_pyramid(base: tuple, deltas: tuple) -> tuple:
    ...
    quantized to six decimal places and converted to finite floats; even an
    integral result is returned as a float (never an int) and negative zero is
    normalized to ``0.0``. Each resulting tile must satisfy ``zmin <= zmax``
    and ``count >= 0``; otherwise ``ValueError`` is raised. The inputs are
    never modified and the returned pyramid can be passed directly to
    :func:`query_tile_pyramid` and :func:`query_tile_pyramid_windows`.
    ...
    with localcontext() as ctx:
        ...
        for base_level, delta_level in zip(base, deltas):
            ...
            for base_tile, delta_tile in zip(base_level, delta_level):
                ...
                if not (math.isfinite(new_zmin) and math.isfinite(new_zmax)):
