...
def _validate_tile_window_stats(tiles) -> None:
    """Validate a flat tuple of window tile-stat 11-tuples.

    Same rules as :func:`_validate_tile_region_stats` plus the cell bounds
    must satisfy ``ix0 <= ix1`` and ``iy0 <= iy1``.
    """
    prev_key = None
    for tile in tiles:
        if not isinstance(tile, tuple) or len(tile) != 11:
...
def assess_tile_window_stats(estimate: tuple, reference: tuple) -> tuple:
    ...
    ``ix0 <= ix1``/``iy0 <= iy1`` and ``zmin``, ``zmax``, ``zmean`` and
    ...
    :raises ValueError: the structure, ordering, duplicates, fields, cell
        bounds, coordinate sets or shared-coordinate bounds of either input are
        bad.
    """
    if not isinstance(estimate, tuple):
        raise TypeError("estimate must be a tuple")
    ...
    _validate_tile_window_stats(estimate)
    _validate_tile_window_stats(reference)
