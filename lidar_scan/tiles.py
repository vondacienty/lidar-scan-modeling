...
def query_tile_window_stats(pyramid: tuple, level: int,
    ...
    """Select all tiles intersecting a cell-index window at a stats-pyramid level.
    ...
    A tile is selected when its closed cell-index rectangle intersects the
    closed window: ``tile.ix1 >= ix_min and tile.ix0 <= ix_max and
    tile.iy1 >= iy_min and tile.iy0 <= iy_max`` (a single shared boundary cell
    therefore counts as an intersection).

    Returns a tuple of the stored 11-tuples at that level, preserving the
    level's existing order; an empty match returns ``()``. The input is never
