...
def aggregate_tile_region_stats(pyramid: tuple, level: int,
    ...
    """Aggregate the tiles of a ``(tx, ty)`` rectangle at a stats-pyramid level.
    ...
    Tiles at that level with ``tx_min <= tx <= tx_max`` and
    ``ty_min <= ty <= ty_max`` are aggregated into
    ``(zmin, zmax, zmean, zsigma, count)``: the minimum ``zmin``, maximum
    ``zmax`` and summed ``count``, with the z statistics combined using
    weights ``w = 1 / zsigma ** 2``: ``zmean = sum(w * zmean) / sum(w)`` and
    ...
    Returns ``None`` when no tile lies within the rectangle. The input is
    never modified or reordered.
    ...
    with localcontext() as ctx:
        ...
        for tile in matched:
            tile_zmin, tile_zmax, tile_zmean, tile_zsigma, tile_count = (
            ...
            if tile_zmax > zmax:
                zmax = tile_zmax
            count += tile_count
            dzsigma = Decimal(str(tile_zsigma))
            weight = Decimal(1) / (dzsigma * dzsigma)
            weights.append(weight)
            weighted_means.append(weight * Decimal(str(tile_zmean)))

        sum_w = _sorted_sum(weights)
        agg_zmean = _sorted_sum(weighted_means) / sum_w
        agg_zsigma = (Decimal(1) / sum_w).sqrt()

        return (_quantize(Decimal(str(zmin))), _quantize(Decimal(str(zmax))),
                _quantize(agg_zmean), _quantize(agg_zsigma), count)
