...
def query_tile_region(pyramid: tuple, level: int,
    ...
    """Select the tiles of ``level`` within an inclusive ``(tx, ty)`` region.

    ``pyramid`` must be an outer tuple as produced by
    :func:`build_tile_pyramid`: each level is a tuple of
    ``(tx, ty, ix0, iy0, ix1, iy1, zmin, zmax, count)`` 9-tuples sorted
    lexicographically by ``(tx, ty)``. ``level``, ``tx_min``, ``ty_min``,
    ``tx_max`` and ``ty_max`` must be non-bool ints, ``level`` must satisfy
    ``0 <= level < len(pyramid)``, and the region must satisfy
    ``tx_min <= tx_max`` and ``ty_min <= ty_max``.

    Returns a tuple of the stored 9-tuples at that level whose coordinates
    satisfy ``tx_min <= tx <= tx_max`` and ``ty_min <= ty <= ty_max``, in the
