"""Deterministic 2D digital elevation model (DEM) from LiDAR points."""
...
def build_dem(points: Iterable[tuple | list], cell_size: int | float = 1.0) -> tuple:
    """Build a deterministic DEM from points with inverse-variance weighting.
    ...
    index; only occupied cells are emitted. Within a cell, z is weighted by
    ``w = 1 / sigma**2`` and the cell sigma is ``sqrt(1 / sum(w))``.
